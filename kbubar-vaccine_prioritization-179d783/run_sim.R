run_sim = function(C, percent_vax, strategy, num_perday, v_e_type, v_e = v_e_constant,
  ...
  Sx_0 <- (N_i - I_0 - E_0 - R_0 - Ix_0 - Ex_0 - Rx_0 - Rv_0)*refuse_vax
...
run_simDynamic = function(C, percent_vax, strategy, num_perday, v_e_type, v_e = v_e_constant,
  ...
  #     everyone else starts in S
  # Runtime: 365 days post start of rollout
  # Rollout:--REMOVED-- Continuous at num_perday% of total pop/day until all vaccines are distributed
  #          so num_perday = 1 implies anticipatory rollout --REMOVED--
  #   num_perday is now timedepentend
  ...
  event_times <- seq(from=0, to= length(C)-1, by=1)
  df <- as.data.frame(deSolve::lsoda(y=compartments_initial, times=t, func=calculate_derivativesDynamic, parms=parameters, events=list(func=move_vaccinated_eventDynamic, time=event_times)))
